...
class Form4aController extends BaseController
{
    ...
    public function getSoalDanJawabanForm4a(Request $request)
    {
        ...
        /**
         * ==================================================
         * AMBIL SOAL (SAMA PERSIS DENGAN getSoalForm4a)
         * ==================================================
         */
        $elemenList = ElemenForm3::with([
            'kukForm3' => function ($query) use ($pkId, $groupNo) {
                $query->where('pk_id', $pkId)
                    ->whereHas('iukForm3', function ($q) use ($pkId, $groupNo) {
                        $q->where('pk_id', $pkId)
                        ->whereRaw('FIND_IN_SET(?, group_no)', [$groupNo])
                    ...
                    ->with([
                        'iukForm3' => function ($q) use ($pkId, $groupNo) {
                            $q->where('pk_id', $pkId)
                            ->whereRaw('FIND_IN_SET(?, group_no)', [$groupNo])
                            ->whereHas('poinForm4')
                            ->with('poinForm4');
        ...
        ->filter(fn ($elemen) => $elemen->kukForm3->isNotEmpty())
        ->values()
        ->map(function ($elemen) use ($form1Id, $userId) {

            $elemen->isi_elemen = preg_replace(
                '/\s+/',
                ' ',
                trim(str_replace(["\t", "\n", "\r"], ' ', $elemen->isi_elemen))
            );

            $elemen->kuk_form3 = $elemen->kukForm3->map(function ($kuk) use ($form1Id, $userId) {

                unset($kuk->kuk_name, $kuk->pk_id);

                $kuk->iuk_form3 = $kuk->iukForm3->map(function ($iuk) use ($form1Id, $userId) {

                    /**
                     * ===============================
                     * QUERY JAWABAN LANGSUNG PER IUK
                     * ===============================
                     */
                    $jawaban = JawabanForm4a::where('form_1_id', $form1Id)
                        ->where('user_id', $userId)
                        ->where('iuk_form3_id', $iuk->iuk_form3_id)
                        ->first();
                    ...
                    return [
                        ...
                        'poin_form4'   => $buildTree(null),
                        'jawaban'      => [
                            'pencapaian' => $jawaban ? (bool) $jawaban->pencapaian : null,
         ...
         * ==================================================
         */
        $totalIuk = $elemenList->sum(fn ($e) =>
            collect($e->kuk_form3)->sum(fn ($k) =>
                collect($k['iuk_form3'])->count()
            )
        );

        $jawabanAll = JawabanForm4a::where('form_1_id', $form1Id)
            ->where('user_id', $userId)
            ->get();

        $jumlahPencapaian    = $jawabanAll->where('pencapaian', 1)->count();
        $jumlahTidakTercapai = $jawabanAll->where('pencapaian', 0)->count();
    ...
    public function ApproveForm4aByAsesi(Request $request, $form4aId)
    {
        ...
        try {
            ...
            // Pastikan $form4a adalah model tunggal
            ...
            if ($dataForm4a instanceof \Illuminate\Support\Collection) {
                $dataForm4a = $dataForm4a->first();
            }
            if (!$dataForm4a) {
                throw new \RuntimeException('Data Form 4A (detail) tidak ditemukan.');
            }

            $form1 = $this->formService->getParentDataByFormId($form1Id);
            Log::info("Form 4A ID: {$form4aId}, Form 1 ID: {$form1Id}");

            // Ambil status Form 4A
            $form4aStatus = $this->formService
                ->getStatusByParentFormIdAndType($form1Id, 'form_4a')
                ->first();

            // Jika Form 4A statusnya Submitted atau InAssessment → Completed
            if ($form4a->status === 'Submitted' || $form4a->status === 'InAssessment') {
                $this->formService->updateForm4a(
                    $form4aId,
                    null, null,
                    'form_4a',
                    null, null,
                ...
                $this->formService->updateProgresDanTrack(
                    ...
                    'Completed',
                    $dataForm4a->asesi_id,
                    'Form 4A telah di-approve oleh Asesor'
                );

                Log::info('Form 4A approved successfully.');
            ...
            if ($isAllForm4Completed) {
                Log::info("Semua Form 4 (A–D) telah lengkap untuk PK ID: {$form4a->pk_id}");
            ...
            return response()->json([
                'success' => true,
                'message' => 'Form 4A berhasil di-approve oleh Asesor',
                'data'    => []
            ]);
